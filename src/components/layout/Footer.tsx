...
import { Send, Mail, MapPin, Calendar, CheckCircle, AlertCircle, ArrowRight } from "lucide-react";
            ...
            <div className="flex items-center justify-center gap-2 mb-8 flex-wrap">
              {["Raw Data", "ETL Pipeline", "Data Warehouse", "Analytics Model", "Executive Insights", "Business Impact"].map((step, i, arr) => (
                <div key={step} className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-blue-200/60 px-2.5 py-1 rounded-lg border border-white/10 bg-white/5">{step}</span>
                  {i < arr.length - 1 && <ArrowRight size={12} className="text-blue-300/40" />}
                </div>
              ))}
            </div>
